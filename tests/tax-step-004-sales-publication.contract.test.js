...
assert.match(publicationService, /result\?\.document\?\.id/, 'Publication evidence must use the Tax registration document result');
assert.doesNotMatch(publicationService, /result\?\.taxDocument\?\.id/, 'Publication must not depend on a non-existent taxDocument result field');
assert.match(publicationService, /replayed:\s*Boolean\(result\?\.replayed\)/, 'Publication evidence must expose replay state');
assert.match(registrationService, /document\s*\}/, 'Tax registration contract must return document evidence');
