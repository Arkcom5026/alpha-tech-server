...
assert.match(repository, /const publishedProductFromSql = \(\) => Prisma\.sql/);
assert.ok((repository.match(/publishedProductFromSql\(\)/g) || []).length >= 5);
assert.doesNotMatch(repository, /const publishedProductFromSql = Prisma\.sql/);
