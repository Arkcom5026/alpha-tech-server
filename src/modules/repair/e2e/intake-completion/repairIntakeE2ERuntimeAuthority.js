...
const TEST_DB_FIXTURE_APPROVAL = 'ALPHATECH_REPAIR_INTAKE_E2E_FIXTURE';
...
const normalizeMode = (value) => {
  ...
  if (!Object.values(MODES).includes(mode)) {
    throw new Error(
      `REPAIR_INTAKE_E2E_DATABASE_MODE must be ${MODES.TEST_DB} or ${MODES.MAIN_TEST_TENANT}.`
    );
  }
  return mode;
};
...
const resolveRepairIntakeE2ERuntimeAuthority = ({ requiresWrite = false } = {}) => {
  const mode = normalizeMode(process.env.REPAIR_INTAKE_E2E_DATABASE_MODE);
  ...
  if (mode === MODES.TEST_DB) {
    ...
    if (
      requiresWrite
      && process.env.REPAIR_INTAKE_E2E_FIXTURE_APPROVAL !== TEST_DB_FIXTURE_APPROVAL
    ) {
      throw new Error(
        `Set REPAIR_INTAKE_E2E_FIXTURE_APPROVAL=${TEST_DB_FIXTURE_APPROVAL} before provisioning.`
      );
    }
  ...
  if (
    requiresWrite
    && process.env.REPAIR_INTAKE_E2E_MAIN_DB_WRITE_APPROVAL !== MAIN_DB_WRITE_APPROVAL
  ) {
    throw new Error(
      `Set REPAIR_INTAKE_E2E_MAIN_DB_WRITE_APPROVAL=${MAIN_DB_WRITE_APPROVAL} before provisioning.`
    );
  }

  const configuredBranchId = Number(
    process.env.REPAIR_INTAKE_E2E_ALLOWED_BRANCH_ID || MAIN_TEST_TENANT.branchId
  );
  const configuredBranchSlug = String(
    process.env.REPAIR_INTAKE_E2E_ALLOWED_BRANCH_SLUG || MAIN_TEST_TENANT.branchSlug
  ).trim();
  ...
  if (
    ...
    throw new Error(
      `Main-DB Repair E2E is fixed to branchId=${MAIN_TEST_TENANT.branchId}, `
