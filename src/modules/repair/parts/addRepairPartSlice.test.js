...
test('add part repository keeps job and stock operations branch-safe', async () => {
  const calls = {};
...
test('add part service writes part, stock decrement and movement atomically', async () => {
  const calls = [];
  const txRepo = {
    findRepairJob(branchId, repairJobId) {
      assert.equal(branchId, 7);
      assert.equal(repairJobId, 31);
      return Promise.resolve({ id: 31, jobNo: 'RP-31', status: 'DIAGNOSING' });
    },
