...
const NEXT_ACTION_BY_STATUS = Object.freeze({
  RECEIVED: 'ตรวจหลักฐานรับเครื่องให้ครบ แล้วเริ่มงานได้เลยหรือเลือกตรวจสอบเมื่อเคสต้องการ',
    ...
    const nextAction = claimContext?.active
      ...
      : claimContext?.handbackPending
        ...
        : workflowStatus === REPAIR_WORKFLOW_STATUS.RECEIVED && preAgreedService?.enabled
          ? 'ลูกค้าอนุมัติให้ซ่อมแล้วโดยไม่ต้องเสนอราคาก่อน ตรวจหลักฐานรับเครื่องให้ครบแล้วเริ่มงานได้ หรือเลือกตรวจสอบเพิ่มเติมเมื่อจำเป็น'
