const customerWarrantyAssetsRepository = require('./customerWarrantyAssetsRepository');
const {
  RepairError,
  RepairFailureCode,
...
class CustomerWarrantyAssetsService {
  constructor(repository = customerWarrantyAssetsRepository) {
  ...
  async execute(actor, customerIdInput) {
    ...
    const findStructured =
      this.repository.findStructuredAssets ||
      this.repository.findCustomerWarrantyStockItems;
    const findSimple =
      this.repository.findSimpleAssets ||
      this.repository.findCustomerWarrantySimpleItems;

    const [stockItems, simpleItems] = await Promise.all([
      findStructured.call(this.repository, actor.branchId, customerId),
      findSimple.call(this.repository, actor.branchId, customerId),
    ]);
  ...
  listForCustomer(actor, customerIdInput) {
    return this.execute(actor, customerIdInput);
  }
}

module.exports = new CustomerWarrantyAssetsService();
module.exports.CustomerWarrantyAssetsService = CustomerWarrantyAssetsService;
module.exports.requirePositiveInteger = requirePositiveInteger;
module.exports.resolveWarranty = resolveWarranty;
module.exports.mapStructuredAsset = mapStructuredAsset;
module.exports.mapSimpleAsset = mapSimpleAsset;
