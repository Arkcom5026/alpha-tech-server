...
test('requires an existing non-terminal repair job linked to stock identity', () => {
  ...
  assert.throws(
    ...
    { code: 'REPAIR_JOB_TERMINAL', status: 409 }
  );
  assert.throws(
    () => assertRepairCanOpenClaim({ status: 'IN_PROGRESS', stockItemId: null, stockItem: null }),
    { code: 'WARRANTY_STOCK_ITEM_REQUIRED', status: 409 }
  );
  assert.doesNotThrow(() =>
...
test('blocks an active claim for the same repair job', () => {
  assert.throws(
    ...
    (error) => {
      assert.equal(error.code, 'ACTIVE_CLAIM_EXISTS');
...
test('requires replacement identity for REPLACED resolution', () => {
  assert.throws(
    ...
    { code: 'WARRANTY_REPLACEMENT_REQUIRED', status: 400 }
  );
  assert.doesNotThrow(() =>
...
test('requires an explicit credit amount for CREDITED resolution including zero', () => {
  assert.throws(
    ...
    { code: 'WARRANTY_CREDIT_REQUIRED', status: 400 }
  );
  assert.doesNotThrow(() =>
