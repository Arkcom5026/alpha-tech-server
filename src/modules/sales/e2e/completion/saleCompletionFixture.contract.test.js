...
assert.match(authority, /environment: 'TEST'[\s\S]*mayMutateOperatorCredential: true/);
assert.match(authority, /environment: 'MAIN_TEST_TENANT'[\s\S]*mayMutateOperatorCredential: false/);
assert.match(fixture, /resolveSaleCompletionE2ERuntimeAuthority\(\{ requiresWrite: true \}\)/);
assert.match(fixture, /authority\.mayMutateOperatorCredential/);
assert.match(fixture, /bcrypt\.hash\(operatorPassword, 12\)/);
