...
const createChallengeInDb = async ({ sessionId, purpose, phoneNormalized, otpHash, expiresAt }, db) => {
  ...
  const rows = await db.$queryRaw(Prisma.sql`
    INSERT INTO "CommerceIdentityChallenge" (
      ...
      ${sessionId}, ${purpose}::"CommerceIdentityChallengePurpose", ${phoneNormalized}, ${otpHash}, 'PENDING',
...
const verifyChallengeAndCreateProofInDb = async ({ challengeId, sessionId, phoneNormalized, proofTokenHash, proofExpiresAt }, db) => {
  ...
  const proofs = await db.$queryRaw(Prisma.sql`
    INSERT INTO "CommerceCommitmentIdentity" (
      ...
      ${challengeId}, ${sessionId}, ${phoneNormalized}, ${proofTokenHash}, CURRENT_TIMESTAMP,
