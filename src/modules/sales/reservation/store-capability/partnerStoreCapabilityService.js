...
const normalizeCommand = (input = {}) => {
  ...
  const serviceAreaMode = enumValue(
    input.serviceAreaMode,
    AREA_MODES,
    'serviceAreaMode',
    deliveryEnabled ? 'ADMIN_AREAS' : 'PICKUP_ONLY'
  );
  const deliveryFeeMode = deliveryEnabled
    ? enumValue(input.deliveryFeeMode, FEE_MODES, 'deliveryFeeMode', 'NEGOTIATED')
    : null;
  const fixedDeliveryFee = deliveryFeeMode === 'FIXED' ? nonNegativeMoney(input.fixedDeliveryFee, 'fixedDeliveryFee') : 0;
  if (deliveryFeeMode === 'FIXED' && fixedDeliveryFee <= 0) {
    fail('PARTNER_STORE_FIXED_FEE_REQUIRED', 'Fixed delivery fee must be greater than zero');
  }
  if (!deliveryEnabled && serviceAreaMode !== 'PICKUP_ONLY') {
    fail('PARTNER_STORE_DELIVERY_DISABLED', 'Delivery-disabled store must use PICKUP_ONLY service area');
  }
  if (deliveryEnabled && serviceAreaMode === 'PICKUP_ONLY') {
    fail('PARTNER_STORE_SERVICE_AREA_REQUIRED', 'Delivery-enabled store requires a delivery service area');
  }

  const serviceAreas = normalizeAreas(input.serviceAreas);
  if (serviceAreaMode === 'ADMIN_AREAS' && serviceAreas.length === 0) {
    fail('PARTNER_STORE_SERVICE_AREA_REQUIRED', 'ADMIN_AREAS requires at least one service area');
  }
  if (serviceAreaMode !== 'ADMIN_AREAS' && serviceAreas.length > 0) {
    fail('PARTNER_STORE_SERVICE_AREA_NOT_ALLOWED', 'Service-area rows are only allowed for ADMIN_AREAS');
  }

  const maxDeliveryDistanceKm = serviceAreaMode === 'DISTANCE' ? Number(input.maxDeliveryDistanceKm) : null;
  if (serviceAreaMode === 'DISTANCE' && (!Number.isFinite(maxDeliveryDistanceKm) || maxDeliveryDistanceKm <= 0)) {
    fail('PARTNER_STORE_DISTANCE_REQUIRED', 'DISTANCE mode requires maxDeliveryDistanceKm greater than zero');
  }

  const storefrontEnabled = bool(input.storefrontEnabled, false);
  const storefrontSlug = text(input.storefrontSlug, 120);
  ...
  if (storefrontSlug && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(storefrontSlug)) {
    fail('PARTNER_STORE_SLUG_INVALID', 'storefrontSlug must use lowercase letters, numbers, and hyphens');
  }
  ...
  return {
    ...
    pickupInstructions: text(input.pickupInstructions, 2000),
    deliveryInstructions: text(input.deliveryInstructions, 2000),
    serviceAreas,
  };
};

const getPartnerStoreCapability = async ({ branchId }) => {
  const capability = await repository.findByBranchId(positiveInt(branchId, 'branchId'));
  return capability || { branchId: Number(branchId), configured: false };
};
