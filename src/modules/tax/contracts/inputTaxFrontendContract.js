...
const InputTaxFrontendErrorCode = Object.freeze({
  ...
  FILING_VAT_AUTHORITY_CONFLICT: 'INPUT_TAX_FILING_VAT_AUTHORITY_CONFLICT',
  FILING_VAT_AUTHORITY_REQUIRED: 'INPUT_TAX_FILING_VAT_AUTHORITY_REQUIRED',
...
const endpoints = Object.freeze([
  ...
  endpoint({ id: 'filing-select', method: 'POST', path: `${TAX_INTAKE_BASE}/input-documents/filing/batches/:batchId/documents/:taxDocumentId/select`, request: Object.freeze({ body: ['branchId'], params: ['batchId', 'taxDocumentId'] }), response: 'InputTaxFilingItem projection with version and replayed; claimed amounts are anchored to locked InputVatRecord authority while eligibility may reduce claimable VAT', capability: 'SELECT_FOR_FILING', replay: 'MUTATION_REPLAY_SAFE', concurrency: Object.freeze({ rowLock: true, versionField: 'version' }), errors: Object.freeze(['INPUT_TAX_FILING_RECONCILIATION_REQUIRED', 'INPUT_TAX_FILING_ELIGIBILITY_REQUIRED', 'INPUT_TAX_FILING_VAT_AUTHORITY_REQUIRED', 'INPUT_TAX_FILING_VAT_AUTHORITY_CONFLICT', 'INPUT_TAX_DOCUMENT_ALREADY_IN_FILING', 'INPUT_TAX_FILING_BATCH_BRANCH_MISMATCH']) }),
