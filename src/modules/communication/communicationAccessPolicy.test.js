...
test('legacy repair employee can use communication without managing branch profiles', () => {
  assert.deepEqual(
...
test('migrated positions require explicit communication capabilities and empty arrays are authoritative', () => {
  assert.deepEqual(
  ...
  assert.deepEqual(
    getCommunicationCapabilities({
      ...
      positionCapabilities: ['communication.operate', 'communication.profile.manage'],
    }),
