...
const printable = async (req, res, next) => { try {
  ...
  const lineIds = document.documentLines.map((line) => line.id);
  const presentations = lineIds.length ? await prisma.consolidatedDeliveryLinePresentation.findMany({
    where: {
      branchId,
      combinedBillingId: id,
      consolidatedDeliveryLineId: { in: lineIds },
    },
    select: {
