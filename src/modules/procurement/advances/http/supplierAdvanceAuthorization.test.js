...
const READ = POSITION_CAPABILITIES.PROCUREMENT_SUPPLIER_ADVANCE_READ;
const MANAGE = POSITION_CAPABILITIES.PROCUREMENT_SUPPLIER_ADVANCE_MANAGE;
const CONTROL = POSITION_CAPABILITIES.PROCUREMENT_SUPPLIER_ADVANCE_CONTROL;

const routeSource = fs.readFileSync(path.join(__dirname, 'supplierAdvanceRoutes.js'), 'utf8');

test('legacy supplier advance authority preserves manager work and owner-only control', () => {
...
test('migrated positions require explicit supplier advance capabilities', () => {
...
test('platform admins retain supplier advance authority', () => {
  const admin = { role: 'ADMIN', positionCapabilities: [] };
  const superAdmin = { role: 'SUPERADMIN', positionCapabilities: [] };

  for (const capability of Object.values(SUPPLIER_ADVANCE_CAPABILITY)) {
    assert.equal(hasCapability(admin, capability), true);
    assert.equal(hasCapability(superAdmin, capability), true);
  }
});

test('supplier advance routes split read, manage and elevated control authority', () => {
  assert.match(routeSource, /router\.get\('\/', requireSupplierAdvanceRead, controller\.list\)/);
  assert.match(routeSource, /router\.post\('\/', requireSupplierAdvanceManage, controller\.create\)/);
  assert.match(routeSource, /router\.post\('\/:advanceId\/apply', requireSupplierAdvanceManage, controller\.apply\)/);
  assert.match(routeSource, /router\.post\('\/:advanceId\/activate', requireSupplierAdvanceControl, controller\.activateLegacy\)/);
  assert.match(routeSource, /router\.post\('\/:advanceId\/void', requireSupplierAdvanceControl, controller\.voidAdvance\)/);
