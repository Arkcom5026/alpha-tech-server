// src/modules/auth/routes/authRuntimeRoutes.js
// Behavior-preserving module-owned runtime for the existing /api/auth contract.
...
const ensureFn = (key) => {
  ...
  throw new Error(`[authRuntimeRoutes] authController.${key} must be a function (got ${typeof fn})`);
};
...
if (typeof addSubEmployee !== 'function') {
  throw new Error('[authRuntimeRoutes] employeeOnboardingController.addSubEmployee must be a function');
}

if (typeof findUserByEmail !== 'function') {
  throw new Error(
    `[authRuntimeRoutes] authController.findUserByEmail must be a function (got ${typeof findUserByEmail})`,
  );
}
