...
class DocumentPurposeLifecycleRepository {
  ...
  findCurrent({ branchId, definitionId }) {
    return this.prisma.documentPurposeDefinition.findFirst({
      where: { id: Number(definitionId), branchId: Number(branchId) },
    })
  }
  ...
  transaction(work) {
    return this.prisma.$transaction((tx) => work(new DocumentPurposeLifecycleRepository(tx)))
  }
