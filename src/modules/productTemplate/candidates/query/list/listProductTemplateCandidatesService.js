...
const buildSearchWhere = (q) => {
  ...
  return {
    OR: [
      ...
      { sourceBranch: { branchCode: { contains: q, mode: 'insensitive' } } },
      { targetTemplateProduct: { name: { contains: q, mode: 'insensitive' } } },
      { targetTemplateBranch: { name: { contains: q, mode: 'insensitive' } } },
      { targetTemplateBranch: { branchCode: { contains: q, mode: 'insensitive' } } },
    ],
  }
}
...
const listCandidates = async ({ user, query = {} }) => {
  ...
  const targetTemplateBranchId = query.targetTemplateBranchId
    ...
    : null
  const reviewerId = query.reviewerId
    ? toPositiveInt(query.reviewerId, 'reviewerId')
    : null
  ...
  for (const group of workloadGroups) {
    const reviewer = workloadMap.get(group.reviewedByEmployeeId) || {
      reviewerId: group.reviewedByEmployeeId,
      assigned: 0,
      pending: 0,
      reviewed: 0,
    }
    ...
    if (['REJECTED', 'PROMOTED', 'MERGED', 'CANCELLED'].includes(group.status)) reviewer.reviewed += count
  ...
  return {
    ...
    summary: {
      total: Object.values(statusSummary).reduce((sum, count) => sum + count, 0),
