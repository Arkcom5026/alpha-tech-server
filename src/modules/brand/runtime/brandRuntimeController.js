const service = require('./brandRuntimeService');

const sendError = (res, error, fallbackMessage, fallbackCode) => {
  const statusCode = error?.statusCode || error?.status || 500;

  if (statusCode >= 500) {
    console.error(`[brandRuntimeController] ${fallbackCode}:`, error);
  }
...
const listProductTypeOptions = async (req, res) => {
  try {
    const payload = await service.listProductTypeOptions({ query: req.query || {}, user: req.user || {} });
...
const listBrands = async (req, res) => {
  try {
    const payload = await service.listBrands({ query: req.query || {}, user: req.user || {} });
    res.set('Cache-Control', 'no-store');
    return res.json(payload);
  } catch (error) {
...
const listBrandDropdowns = async (req, res) => {
  try {
    const payload = await service.listDropdownBrands({ query: req.query || {}, user: req.user || {} });
    res.set('Cache-Control', 'no-store');
    return res.json(payload);
  } catch (error) {
...
const createBrand = async (req, res) => {
  try {
    return res.status(201).json(await service.createBrand({ payload: req.body || {} }));
  } catch (error) {
...
const updateBrand = async (req, res) => {
  try {
    return res.json(await service.updateBrand({ id: req.params?.id, payload: req.body || {} }));
  } catch (error) {
...
const toggleBrand = async (req, res) => {
  try {
    return res.json(await service.toggleBrand({ id: req.params?.id, payload: req.body || {} }));
  } catch (error) {
...
const listProductTypeBrands = async (req, res) => {
  try {
    const payload = await service.listProductTypeBrandLinks({ query: req.query || {}, user: req.user || {} });
    res.set('Cache-Control', 'no-store');
    return res.json(payload);
  } catch (error) {
...
const attachBrandToProductType = async (req, res) => {
  try {
    const payload = await service.attachBrandToProductType({ payload: req.body || {}, user: req.user || {} });
    return res.status(201).json({ message: 'เพิ่ม mapping แบรนด์กับประเภทสินค้าสำเร็จ', data: payload });
  } catch (error) {
...
const detachBrandFromProductType = async (req, res) => {
  try {
    const payload = await service.detachBrandFromProductType({ id: req.params?.id, user: req.user || {} });
    return res.json({ message: 'ลบ mapping แบรนด์กับประเภทสินค้าสำเร็จ', data: payload });
  } catch (error) {
