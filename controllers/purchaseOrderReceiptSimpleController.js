// ================================
// server/controllers/purchaseOrderReceiptSimpleController.js
// CommonJS • Controller + (inlined service) for PO‑tied Simple receipts (no SN)
// Mount: app.use('/api/po-receipts/simple', routes)
// Routes:
//   POST /api/po-receipts/simple          -> create (persist)
//   POST /api/po-receipts/simple/preview  -> preview (calculate only)
// ================================
...
// ------------------------------
// Document code helpers (sequential like purchaseOrderController)
// ------------------------------
...
// Guards
// ------------------------------
const guardQuickReceive = async (req, res) => {
  const user = req.user || {};
  ...
  const features = branch && typeof branch.features === 'object' ? branch.features : {};
  const quickCfg = features && typeof features.quickReceive === 'object' ? features.quickReceive : {};
  const allowQuickReceive = quickCfg.enabled !== false; // default = true
  if (!allowQuickReceive) {
    res.status(403).json({ code: 'QUICK_RECEIVE_DISABLED', message: 'สาขานี้ปิดการใช้งาน Quick Receive' });
    return false;
  }
  ...
  if (managerPin) {
    ...
    if (!hash) {
      res.status(401).json({ code: 'PIN_INVALID', message: 'ไม่มีสิทธิ์หรือยังไม่ได้ตั้งค่า Manager PIN' });
      return false;
    }
    const ok = await bcrypt.compare(String(managerPin), hash);
    if (!ok) {
      res.status(401).json({ code: 'PIN_INVALID', message: 'Manager PIN ไม่ถูกต้อง' });
      return false;
    }
    req.quickReceive = { ...(req.quickReceive || {}), pinApproved: true };
  }
  return true;
};

// ------------------------------
// Optional helpers (from service): date range, inventory txn, payment
// ------------------------------
const todayRange = () => {
  const now = new Date();
    ...
    try {
      const txn = await tx.inventoryTransaction.create({
        data: {
          ...
          refType: 'PO_RECEIPT',
          refId: receiptId,
          note: 'Simple Receipt (PO)',
          createdBy: userId,
        },
      });
      ids.push(txn.id);
    } catch (err) {
      // ถ้าไม่มีตาราง InventoryTransaction ในช่วงแรก สามารถคอมเมนต์บล็อกนี้ชั่วคราวได้
  ...
  try {
    ...
    const pay = await tx.payment.create({
      data: {
        ...
        note: payment.note || 'Simple Receipt (PO)',
        refType: 'PO_RECEIPT',
        refId: receiptId,
        createdBy: userId,
      },
...
// ------------------------------
// POST /api/po-receipts/simple (persist)
// ------------------------------
const create = async (req, res) => {
  try {
    ...
    const passed = await guardQuickReceive(req, res);
    if (!passed) return;
    ...
    for (let attempt = 0; attempt < 5 && !result; attempt++) {
      ...
      try {
        result = await prisma.$transaction(async (tx) => {
          ...
          if (!employee) {
            const err = new Error('ไม่พบข้อมูลพนักงานของผู้ใช้งานในสาขานี้');
            err.code = 'VALIDATION_ERROR';
            throw err;
          }

          // 1) PurchaseOrder (หัวใบสั่งซื้อสำหรับอ้างอิงใบรับเข้า)
          ...
          // 2) PurchaseOrderItem (ต่อสินค้า SIMPLE)
          const poItems = [];
          for (const it of itemsSafe) {
            const poi = await tx.purchaseOrderItem.create({
              data: {
                ...
                productId: it.productId,
                quantity: it.qty,          // Int
                costPrice: it.unitCost,     // ไม่รวม VAT
              },
            });
            poItems.push(poi);
          }

          // 3) PurchaseOrderReceipt (หัวใบรับเข้า)
          const receipt = await tx.purchaseOrderReceipt.create({
            data: {
              ...
              vatRate,
              totalAmount: summary.total,   // รวม VAT
              note: note ?? '',
            },
          });

          // 4) ReceiptItems + update receivedQuantity
          const receiptItems = [];
          for (let i = 0; i < itemsSafe.length; i++) {
            const it = itemsSafe[i];
            const poi = poItems[i];
            const pri = await tx.purchaseOrderReceiptItem.create({
              data: {
          ...
          // 5) อัปเดตสต๊อก (BranchInventory) + Movement
          for (const it of itemsSafe) {
          ...
          // 6) (ทางเลือก) Inventory transactions & Payment
          let inventoryTxnIds = [];
    ...
    if (!result) {
      const err = new Error('ไม่สามารถสร้างรหัสเอกสารที่ไม่ซ้ำได้');
      err.code = 'DOC_CODE_ERROR';
      throw err;
    }
    ...
    return res.status(201).json({
      ...
      note: note ?? '',
      vatRate,
      limits: await readQuickReceiveLimits(branchId),
      summary,
      ids: {
    ...
    if (code === 'QUICK_RECEIVE_DISABLED') return res.status(403).json({ message: 'สาขานี้ปิดการใช้งาน Quick Receive' });
    ...
    console.error('[purchaseOrderReceiptSimpleController.create] Unhandled error:', err);
    return res.status(500).json({ message: 'เกิดข้อผิดพลาดภายในระบบ' });
  }
};

// ------------------------------
// POST /api/po-receipts/simple/preview (calculate only)
// ------------------------------
const preview = async (req, res) => {
  try {
    ...
    console.error('[purchaseOrderReceiptSimpleController.preview] Unhandled error:', err);
    return res.status(500).json({ message: 'เกิดข้อผิดพลาดภายในระบบ' });
  }
};
