...
-- Additive durable branch policy foundation reconstructed from accepted PR #48 evidence.

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'OnlineDeliveryFeeMode') THEN
    CREATE TYPE "OnlineDeliveryFeeMode" AS ENUM ('FREE', 'FIXED', 'NEGOTIATED');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'StoreServiceAreaMode') THEN
    CREATE TYPE "StoreServiceAreaMode" AS ENUM ('PICKUP_ONLY', 'ADMIN_AREAS', 'DISTANCE', 'NATIONWIDE');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'StoreServiceAreaType') THEN
    CREATE TYPE "StoreServiceAreaType" AS ENUM ('PROVINCE', 'DISTRICT', 'SUBDISTRICT', 'POSTAL_CODE');
  END IF;
END $$;

CREATE TABLE IF NOT EXISTS "PartnerStoreCapability" (
...
CREATE TABLE IF NOT EXISTS "PartnerStoreServiceArea" (
  "id" SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS "PartnerStoreCapability_storefrontEnabled_idx"
  ON "PartnerStoreCapability"("storefrontEnabled");

CREATE INDEX IF NOT EXISTS "PartnerStoreCapability_pickup_delivery_idx"
  ON "PartnerStoreCapability"("pickupEnabled", "deliveryEnabled");

CREATE INDEX IF NOT EXISTS "PartnerStoreServiceArea_lookup_idx"
