...
function Write-Utf8NoBomRetry([string]$Path,[string]$Content){
  ...
  $last=$null
  for($attempt=1;$attempt-le20;$attempt++){
    try{
      [IO.File]::WriteAllText($Path,$Content,$encoding)
      return
    }catch [IO.IOException]{
      $last=$_.Exception
      if($attempt-ge20){break}
...
try{
  ...
  Write-Warning "Wave 4A apply failed; restoring Prisma files from external recovery: $backup"
  git restore --source=HEAD --worktree -- prisma/schema.prisma
  foreach($path in $targets.Keys){if(Test-Path $path){Remove-Item -LiteralPath $path -Force -ErrorAction SilentlyContinue}}
  throw
}
