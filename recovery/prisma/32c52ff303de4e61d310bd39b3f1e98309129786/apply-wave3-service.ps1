...
function Get-Block([string]$Text,[string]$Kind,[string]$Name){$m=[regex]::Match($Text,'(?m)^'+[regex]::Escape($Kind)+'\s+'+[regex]::Escape($Name)+'\s*\{');if(!$m.Success){throw "Missing $Kind $Name"};$b=$Text.IndexOf('{',$m.Index);$d=0;$e=-1;for($i=$b;$i-lt$Text.Length;$i++){if($Text[$i]-eq'{'){$d++}elseif($Text[$i]-eq'}'){$d--;if($d-eq0){$e=$i+1;break}}};if($e-lt0){throw "Unclosed $Kind $Name"};[pscustomobject]@{Start=$m.Index;End=$e;Text=$Text.Substring($m.Index,$e-$m.Index)}}
...
$backup=Join-Path $root ('prisma-wave3-preapply-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Host 'Wave 3 service-domain candidate created.'
